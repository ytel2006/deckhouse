...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(url, {
    ...
    .then(data => {
      for (const item of data.deployments) {
        ...
        const date = new Date(Date.parse(getValueCell(item, 'updated')))


        channel.innerText = getValueCell(item, 'channel');
        version.innerText = getValueCell(item, 'version');
        link.href = getValueCell(item, 'log');
        link.innerText = getValueCell(item, 'state');
  ...
  function getValueCell(obj, myKey) {
    if (myKey in obj) {
      return obj[myKey];
    }
  }
